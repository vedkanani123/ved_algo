...
import { ConsoleNav, DangerAction, EmptyState, ExpiryForm, LicenseKey, LicenseStatus, MfaGate } from '../../../components/console-ui';
...
export default async function LicenseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const owner = await requireOwner({ mfa: false });
  const { id } = await params;
  const license = await getLicense(id);
  const keyForResponse = owner.mfaVerified ? license.licenseKey : undefined;
  const publicRecordId = `LIC-${id.slice(0, 8).toUpperCase()}`;
  return <div className="app-shell"><ConsoleNav active="licenses" mfaVerified={owner.mfaVerified} /><main className="main"><header className="detail-header"><div><p className="eyebrow">License record / {id}</p><h1>{owner.mfaVerified ? license.licenseKey.slice(0, 19) : publicRecordId}</h1></div><div className="page-actions"><LicenseStatus status={license.status} />{owner.mfaVerified && <a className="button button--amber" href={`/api/licenses/${id}/set`}>↓ Download .set</a>}</div></header>
    <section className="trust-strip" aria-label="Current authorization chain"><div className="trust-node"><span>01 / License</span><strong>{owner.mfaVerified ? license.licenseKey.slice(0, 14) : publicRecordId}</strong></div><i className="trust-link" /><div className="trust-node"><span>02 / Trading account</span><strong>{license.allowed_account ?? 'Awaiting first bind'}</strong></div><i className="trust-link" /><div className="trust-node"><span>03 / Terminal device</span><strong>{license.bound_device_fingerprint ?? 'Awaiting first bind'}</strong></div><i className="trust-link" /><div className="trust-node"><span>04 / Last heartbeat</span><strong>{license.last_seen_at ? new Date(license.last_seen_at).toLocaleString('en-IN') : 'No heartbeat received'}</strong></div></section>
    <div className="detail-grid"><div><section className="section"><header className="panel-header"><div><p className="eyebrow">Credential material</p><h2>License key</h2></div></header><div className="key-card"><LicenseKey value={keyForResponse} /></div><MfaGate verified={owner.mfaVerified} /></section>
