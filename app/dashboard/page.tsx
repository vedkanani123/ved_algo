...
export default async function DashboardPage() {
  ...
  return <div className="app-shell"><ConsoleNav active="dashboard" /><main className="main">
    ...
    <aside><header className="panel-header"><div><p className="eyebrow">Terminal wire / live</p><h2>Recent heartbeats</h2></div></header>{heartbeats.length === 0 ? <EmptyState title="No terminal heartbeats">Accepted EA handshakes will appear here with account, balance, equity, positions, and deals.</EmptyState> : <div className="event-stream">{heartbeats.map((heartbeat) => { const license = licenses.find((item) => item.id === heartbeat.license_id); const telemetry = heartbeat.telemetry ?? {}; return <article className="event" key={heartbeat.id}><i className="event__rail" /><div><div className="event__title">{license?.customer_label ?? 'License terminal'} accepted</div><div className="event__detail">ACC {heartbeat.account_number} · BAL {telemetry.balance?.toFixed(2) ?? '—'} · EQ {telemetry.equity?.toFixed(2) ?? '—'} · POS {telemetry.openPositions ?? 0} · DEALS {telemetry.dealsToday ?? 0}</div></div><time>{new Date(heartbeat.received_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</time></article>; })}</div>}</aside></div>
  </main></div>;
}
